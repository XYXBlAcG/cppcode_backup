...
const int N = 2e5 + 5;
int n, m, ss, tt, s, t, head[N], pedge = 1, vl[N], val, dis[N], vis[N], fl[N], path[N], tmpres, result, fires, las, cnt[N];
struct Edge{
    int nxt, ver, w, f;
}edge[N];
...
signed main(){
    ...
    if(ans.flow != val){
        ...
        // for (int i = 1; i <= m; i++) ans.val += res[i] * edge[i << 1].f;
        ans.val += tmpres;
        printf("%lld\n", result = ans.val);
        ...
        ek.ek();
        printf("%lld %lld\n", las = ans.flow + tmp, fires = ans.val + result);
        s = tt, t = ss;
        ek.ek();
        printf("%lld %lld\n", las - ans.flow, fires + ans.val);
    }
    return 0;
}